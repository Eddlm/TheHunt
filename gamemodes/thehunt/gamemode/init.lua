...
concommand.Add("h_dropweapon", function(ply, command, arguments)
	if GetConVarString("h_weight_system") == "1" then 
...
function GM:InitPostEntity()
	...
	timer.Simple(1, function() 
		...
		if GetConVarString("h_STALKER_sweps") == "1" then
			...
			table.insert(MEDIUMWEAPONS, ""..table.Random(STALKER_SWEPS).."")
			table.insert(MEDIUMWEAPONS, ""..table.Random(STALKER_SWEPS).."")
			while table.Count(MEDIUMWEAPONS) > 8 do 
				table.remove(MEDIUMWEAPONS, math.random(1, table.Count(MEDIUMWEAPONS))) 
			end
		...
		if GetConVarString("h_personalized_sweps") ~= "" then
		...
		if GetConVarString("h_silent_kill_rewards") ~= "" then
...
function GM:OnNPCKilled(victim, killer, weapon)
	...
	if victim:GetClass() == "npc_turret_floor" then
		...
		table.insert(zonescovered, victim:GetPos()+Vector(0, 0, 30))
		print("Patrol zone added")
	end
		...
		if killer:IsPlayer() then
			...
			if !victim:GetEnemy() then
				...
				if killer.sex == "male" then 
					killer:EmitSound(table.Random(malecomments), 100, 100) 
				else
...
function ScaleNPCDamage(damaged, hitgroup, damage)
	table.foreach(MainEnemiesDamage, function(key, value)
		if damaged:GetClass() == value  then
			if damage:IsDamageType(8) or damage:GetAttacker():GetClass() == damaged:GetClass() then  -- flee from fire and friendly fire
	...
	if damaged:GetClass() == "npc_gunship" then
		if damaged:Health() <= damage:GetDamage() then
			...
			timer.Create("HeliCoolDown", 500, 1, function() 
				HeliCanSpawn = true 
				print("Helicopters can spawn again.") 
			end)
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	...
	table.insert(zonescovered, ply:GetPos()+Vector(0, 0, 30)) 
	print("[The Hunt]: Patrol zone added")
